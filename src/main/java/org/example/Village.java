...
public class Village {
    ...
    public void AddWorker(String name, String occupation) {
        if(occupationHashMap.containsKey(occupation)) {
            //Added if-statement, so you can't add to many workers in the village with methods in just the village-class
    ...
    public void AddProject(String name) {
        if (possibleProjects.containsKey(name)) {
            ...
            //Change limits in if-statement to >= from >
    ...
    private void FeedWorkers() {
        for (Worker worker: workers) {
            if (food > 0 && worker.isAlive()) {
                ...
                if (worker.isAlive()) {
                    //If  there is no food, the worker gets hungry
